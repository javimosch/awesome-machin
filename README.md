...
- **[machin-demo-3d](https://github.com/javimosch/machin-demo-3d)** — a real-time **3D** scene: a camera orbits a ring of bobbing cubes on a grid. The 3D dogfood — raylib's `Camera3D` is a by-value struct of three `Vector3`s, which drove **FFI nested cstructs** (a `cstruct` field that is another `cstruct`, marshaled recursively) into machin, so `BeginMode3D(Camera3D{Vector3{...}, ...})` works. Orbit trig via libm (`extern "m"`) flags native math as the next gap.
